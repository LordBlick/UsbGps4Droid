...
public abstract class GpsInputReader {
	...
	final private int looksLikeSirf(final int startPos) {
		...
		/* Payload size */
		payloadSize = ((buf[startPos+2]&0xff) << 8) | (buf[startPos+3]&0xff);
		if (payloadSize > 1023) {
		...
		/* Checksum */
		providedCsum = ((buf[startPos+4+payloadSize]&0xff) << 8) |
				(buf[startPos+4+payloadSize+1]&0xff);
	...
	public void loop() throws IOException {
		...
		mainloop: for(;;) {
			...
			if ((truncatedMsgPos == 0) && (this.pos == this.buf.length)) {
				...
				if (truncatedMsgPos < 0) {
					Log.d(TAG, String.format("Skipped %d garbage bytes", this.buf.length));
				}
			}
			if (truncatedMsgPos < 0) {
				this.pos = 0;
